function ImagenSuperpuestaB (DireccionImagen, mapa, xintervalo, yintervalo,TamHaz, AlphaVal, serieColor, tamPixel,  NombreCampo, Intervalo)
...
for e = 1:length(PuntosX)
    ...
    %hold on
	if ~isnan(B(e, 3))
		s = circles(B(e, 1),B(e, 2),B(e, 4),'Color', genColor(min(Intervalo), max(Intervalo), B(e, 3), serieColor), 'edgecolor', [0 0 0]);
		%hold off
		%alpha(.1)
		alpha(s,AlphaVal)
	end
...
%colormap gray
h = colorbar;
caxis([min(Intervalo) max(Intervalo)])
